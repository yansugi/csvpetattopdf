...
public sealed class FormulaEvaluatorTests
{
    ...
    public void TryEvaluate_列同士の掛け算を計算する()
    {
        ...
        Assert.True(FormulaEvaluator.TryEvaluate("{単価}*{数量}", rowData, 1, out double result));
    ...
    public void TryEvaluate_四則演算と括弧の優先順位を正しく処理する()
    {
        ...
        Assert.True(FormulaEvaluator.TryEvaluate("({A}+{B})*{C}", rowData, 1, out double result));
    ...
    public void TryEvaluate_行番号トークンを1始まりの連番として使える()
    {
        ...
        Assert.True(FormulaEvaluator.TryEvaluate("{行番号}*100", rowData, 5, out double result));
    ...
    {
        var rowData = new Dictionary<string, string>();
        Assert.True(FormulaEvaluator.TryEvaluate("1 + 2 * 3", rowData, 1, out double result));
    ...
    public void TryEvaluate_存在しない列を参照した場合は失敗する()
    {
        ...
        Assert.False(FormulaEvaluator.TryEvaluate("{単価}*{存在しない列}", rowData, 1, out _));
    }
    ...
    public void TryEvaluate_列の値が数値でない場合は失敗する()
    {
        ...
        Assert.False(FormulaEvaluator.TryEvaluate("{氏名}+1", rowData, 1, out _));
    }
    ...
    public void TryEvaluate_0除算は失敗する()
    {
        ...
        Assert.False(FormulaEvaluator.TryEvaluate("{A}/{B}", rowData, 1, out _));
    }
    ...
    public void TryEvaluate_構文エラーの数式は失敗する()
    {
        ...
        Assert.False(FormulaEvaluator.TryEvaluate("{A}+*2", rowData, 1, out _));
    }
    ...
    {
        var rowData = new Dictionary<string, string> { ["A"] = "10" };
        Assert.False(FormulaEvaluator.TryEvaluate("({A}+1", rowData, 1, out _));
    }
}
