...
CSVの各行データをPDFテンプレート上の指定位置に差し込み、PDFを一括生成するWindows向けの完全オフライン・デスクトップツール。

ありそうでなかった、PDFに直接データをレイアウトして帳票を作成するツールです。

画面のスクリーンショット付きの使い方は [操作マニュアル](docs/操作マニュアル.md) を参照してください。配布exeと同じフォルダにもPDF版(`manual/操作マニュアル.pdf`)が同梱されています。
...
- **QRコード** — CSV列の値やURL等をQRコードとして配置。誤り訂正レベルの指定にも対応。
- **配布はexe1つ** — `dotnet publish`でWindows向け自己完結型の単一exeとして書き出せます。

## 主な機能

- PDFテンプレートへのドラッグ&ドロップでのフィールド配置(自由テキスト・CSV列・計算式・QRコード)
...
初めて使う場合は、同梱の `samples/` フォルダ(配布exeと同じ場所に展開されます)にあるサンプルで動作を確認できます。

- `samples/sample-single.pdf` — 「氏名」「会社名」「郵便番号」「住所」「金額」「発行日」「備考」のラベルを印刷した単票(請求書風)のPDFテンプレート
- `samples/dummy_data.csv` — 上記のラベルに対応する列を持つ5行分のサンプルCSV

ホーム画面で「新規プロジェクトを作成」から `sample-single.pdf` を選び、配置エディタで `dummy_data.csv` を読み込んで、各ラベルの右側の空きスペースに対応するCSV列をドラッグ&ドロップして配置してみてください。
